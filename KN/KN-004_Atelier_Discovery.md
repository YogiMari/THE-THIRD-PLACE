KN-004 Atelier Discovery
# KN-004 Atelier Discovery

## Purpose

Atelier Discovery は、

THE THIRD PLACEに関わる「現在」を調査・観測するためのリサーチメディアである。

市場、

ブランド、

価格、

流通、

コミュニティ、

最新動向を継続的に観測し、

Projectに必要な情報だけを収集・整理する。

目的は情報量ではない。

Projectに必要な「今」を正しく把握し、

意思決定を支えることである。

---

# Editorial Philosophy

Atelier Discovery は、

ブランドを宣伝する媒体ではない。

新製品を紹介する媒体でもない。

THE THIRD PLACEに必要な価値だけを抽出し、

Projectへ還元するためのリサーチメディアである。

評価基準は、

人気でも、

限定でも、

希少性でもない。

唯一の判断基準は、

Design Bibleとの親和性である。

---

# Scope

Atelier Discovery が扱う対象

- Garage Brands
- Outdoor Brands
- Camping Gear
- Market
- Community
- SNS
- Events
- Product Releases

---

# Must Buy Dashboard

## Purpose

Must Buy Dashboard は、

THE THIRD PLACE における最優先購入対象を継続監視し、

購入機会を逃さないための常設ダッシュボードである。

Atelier Discovery の冒頭に掲載し、

毎号必ず更新する。

---

## Watch List

| Priority | Brand | Product |
|----------|-------|---------|
| S | DEVISE WORKS × ANCAM | ANO D TENBAN |
| S | WANTKEY CAMP × LOCKFIELD EQUIPMENT | SC HANDLE WANTKEY Exclusive |
| S | rove troupe | ECHO LAMP |
| S | wildingout | LF1984 |
| S | KURASHI MADE | DOME LOOK |

---

## Dashboard Items

毎号、以下を更新する。

- Status
- Drop Probability
- Confidence
- Heat
- Last Update
- Notes

---

### Status

現在の状況を示す。

- Monitoring
- Teaser
- Announced
- Reservation
- Lottery
- On Sale
- Sold Out
- Discontinued

---

### Drop Probability

今後30日程度の販売・発表可能性。

以下を根拠として算出する。

- ブランド公式発表
- 販売周期
- 過去実績
- イベント情報
- 公式SNS

推測ではなく、

確認できる情報を基準とする。

---

### Confidence

Drop Probability の信頼度。

- High
- Medium
- Low

---

### Heat

ブランド活動の活発さ。

販売確率ではなく、

ブランド全体の動向を示す。

- Active
- Moderate
- Quiet

---

### Last Update

最後に確認した動きを記録する。

---

### Notes

購入判断に必要な補足情報を記録する。

---

# Regular Contents

## Market Watch

市場価格を継続調査する。

対象

- 新品価格
- 中古価格
- 相場推移
- プレミア状況

価格だけではなく、

市場全体の動向を記録する。

---

## Release Watch

ブランドの最新動向。

対象

- 新製品
- 抽選販売
- 再販
- 限定販売
- イベント
- Exhibition
- Pop-up Store

---

## Brand News

ブランドから発信された重要情報。

対象

- ブランドニュース
- コラボレーション
- 新ブランド
- ブランド方針変更
- デザイナー情報

速報性を重視する。

詳細な特集は Beyond Journey で扱う。

---

## Availability Report

現在の入手状況。

対象

- 在庫
- 販売方法
- 入手難易度
- 販売チャネル

Projectの購入判断を支援する。

---

## Must Buy Watch

OP-005 Acquisition Strategy に登録された

Must Buy Item を継続監視する。

対象

- 新品販売
- 未使用中古
- 抽選
- 正規販売
- イベント販売

---

## Social Discovery

毎号必ず掲載する。

### Featured Brand

毎号一つ、

ガレージブランド公式SNSを紹介する。

対象は、

ブランドそのものではなく、

現在の活動、

世界観、

発信内容である。

---

### Featured Community

毎号一人、

THE THIRD PLACEとの親和性が高い人物を紹介する。

対象

- キャンパー
- 写真家
- クリエイター
- ガレージブランドオーナー
- デザイナー

選定基準は、

知名度ではない。

価値観である。

---

## Spotlight

毎号、

「今月、この一枚」

を掲載する。

編集部が選んだ写真を一枚取り上げ、

なぜ心を動かすのかを短く解説する。

対象は、

ブランド、

人物、

風景を問わない。

---

# Editorial Rules

Atelier Discovery は、

「現在」を扱う。

Projectを変更する権限は持たない。

Discoveryで得られた情報は、

必要に応じて

- OP-005 Acquisition Strategy
- MD-004 Equipment Registry
- KN-001 Heritage Chronicle
- KN-003 Beyond Journey

へ反映される。

---

情報は、

一次情報を最優先とする。

優先順位

1. ブランド公式
2. メーカー公式
3. 正規販売店
4. 実物確認
5. コミュニティ情報

コミュニティ情報は、

公式情報と区別して扱う。

---

# Relationship

Atelier Discoveryは、

Documentation Systemの中で、

唯一「現在」を観測する文書である。

```text
Today

↓

Research

↓

Discovery

↓

Decision

↓

Project
```

---

# Closing Statement

良い判断は、

良い情報から生まれる。

Atelier Discoveryは、

情報を集めるためではない。

THE THIRD PLACEに必要な価値だけを見極め、

Projectへ届けるために存在する。

現在を知ることは、

未来を育てる第一歩である。

---

## Document Renumbering Note

本文書は、2026-09-19付のプロジェクト全体の文書番号再編により、TM-002からKN-004へ番号を変更した。Editorial Rules及びMust Buy Watch内の他文書参照を新ID体系へ更新した。その他の内容に変更はない。旧ID: TM-002。

---

# THE THIRD PLACE
